...
def pdfReader(pdf_path):
    with open(pdf_path, 'rb') as pdfFileObject:
        pdfReader = PdfReader(pdfFileObject)  # Use PdfReader instead of PdfFileReader
        count = len(pdfReader.pages)  # Adjust to new syntax
        ...
        for i in range(start_page, end_page + 1):
            page = pdfReader.pages[i]  # Adjusted to new syntax
            extracted_text += page.extract_text()  # Changed to extract_text()
